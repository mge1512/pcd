...
| build | required | `Makefile` | Targets: `build`, `test`, `install`, `clean`, `man`. CGO_ENABLED=0. `man` target: `pandoc mcp-server-{n}.1.md -s -t man -o mcp-server-{n}.1`. |
...
- `%build` must include: `pandoc mcp-server-{n}.1.md -s -t man -o mcp-server-{n}.1`
- `%files` must include: `%{_mandir}/man1/mcp-server-{n}.1*`

**debian/control:**
- `Build-Depends` must include `pandoc`

**debian/rules:**
- Must include man page build step: `pandoc mcp-server-{n}.1.md -s -t man -o mcp-server-{n}.1`
- Man page must be installed to `usr/share/man/man1/mcp-server-{n}.1`
