In the INPUT directory, I am providing three files:

1. templates/mcp-server.template.md — a deployment template that defines
   the conventions, constraints, and defaults for this type of component
   under the Post-Coding Development Paradigm.

2. mcp-server-pcdp.md — a specification for a component, written in the
   Post-Coding Development Paradigm format.

3. mcp-server-pcdp-translation-prompt.md (this file)
...
**Embed prompt files as compiled-in constants.**
The spec's `## TOOLCHAIN-CONSTRAINTS` section lists files under `EMBED-FILES`.
For each entry, read the source file from the INPUT directory and write its
full text as a Go string constant with the given name in the given package.
Use explicit `const` or `var` string literals — do not use `go:embed`.
