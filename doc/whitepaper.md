...
This gap is structural, not a capability gap that better models will close, and the evidence for that comes from outside PCD entirely. A 2026 NBER study tracking more than 100,000 GitHub developers found that successive generations of AI coding tools raised coding activity sharply - commits rose 180 percent for autonomous agents - yet the gain attenuated to only 30 percent for shipped releases, with an estimated elasticity of substitution of 0.25 between AI and human effort, meaning AI and human work are complements rather than substitutes \[Demirer2026\]. Narayanan and Kapoor name the mechanism: software work is a decide-execute-deliver sandwich, and AI compresses only the middle, execute layer, while deciding what to build and being accountable for what ships resist automation \[Narayanan2026\]. The point is forty years old in essence - Brooks separated the essential complexity of specifying correct behaviour from the accidental complexity of expressing it, and held that tooling reduces only the latter \[Brooks1986\]. PCD rests on precisely this premise: because writing code was never the bottleneck, code is demoted to a generated artifact and the human investment moves to the layer that resists automation, the specification. This convergence is independent confirmation of the problem framing, not validation of PCD specifically; the cited authors have never heard of it.
...
AI translation is probabilistic, and the standard mitigations — structured
specifications, pre-translation linting, tests-first discipline, executable
...
Dual-LLM verification is the appropriate escalation when one or more of the
following applies:

- The component has `Safety-Level` above QM (ASIL-B and above, DAL-B and
  above, or equivalent).
- The component has `Verification:` other than `none` (Lean 4, F*, Dafny,
  or custom).
- The specification is novel and the ambiguity surface has not been
  characterised in earlier translation runs.
- The component is part of the PCD toolchain itself, where a regression
...
**Milestone-level dual-LLM.** For specifications partitioned into milestones,
dual-LLM mode may be applied per milestone rather than across the full
specification. This is the right framing for large components: the
scaffold milestone (where the package structure and interface shapes are
...
language only. A Go implementation tested by Python tests would require
both runtimes in the certified image, doubling the supply-chain surface
...
Test directories are named after the LLM that produced them:
`independent_tests/<llm-name>/`. Names are lowercase, hyphen-separated,
and contain no version-decimal suffix (`claude-sonnet-4-5`, not
`Claude-Sonnet-4.5`).

This naming convention makes the directory listing under
`independent_tests/` itself an audit artefact: a reviewer can tell which
models were involved without parsing any metadata. Dates and full version
strings live in TRANSLATION_REPORT.md and TEST_REPORT.md, not in directory
names — otherwise the directory accumulates a copy on every regeneration.

### Cost Asymmetry

Single-LLM mode costs one translation run. Dual-LLM mode costs two runs
plus one additional test-execution pass. For a component where a runtime
defect has safety, security, or regulatory consequences, the additional
cost is justified by the spec-ambiguity probe and the independent test
suite. For components without such consequences, the cost is overhead.
The operational gate keeps the choice explicit and the audit trail honest.

### What Was Removed from Earlier Versions

Earlier versions of this paradigm (v0.3.0 through v0.3.22) described a
four-cell cross-validation matrix: two implementations, two test suites,
four test runs. This produced more matrix combinations than diagnostic
value — when implementations agreed, three of the four cells were
...
Key differences from Post-Coding Development: SPDD permits hand-editing of generated code under an explicit *refactor-then-sync* path that flows changes from code back to the prompt; its Operations section specifies method signatures and parameter types, making the prompt language-bound rather than language-neutral; and it does not address formal verification, supply-chain provenance, or regulated-domain certification. Birgitta Böckeler categorises SPDD as a *spec-anchored* approach; PCD is *spec-only*. The two paradigms target different domains and are complementary: SPDD targets enterprise IT delivery where prompts and code evolve together; PCD targets regulated and long-lived components where the specification must remain the sole audit point. The convergence — SPDD, AWS Kiro, AIUP, and PCD all arriving at structured-spec-before-code in the same window — is independent confirmation of the problem framing.

**AI Unified Process (AIUP), Simon Martinelli (Industry, 2026)**
In 2026, Simon Martinelli published the *AI Unified Process* at unifiedprocess.ai — a spec-driven, AI-native methodology explicitly inspired by the Rational Unified Process. AIUP organises a project through four phases (Inception, Elaboration, Construction, Transition), two workflows (Greenfield from requirements, Brownfield by reverse-engineering specifications from running code), and six core principles centred on requirements-driven generation and test-protected regeneration. The spec primitive is the *Use Case*, written as `UC-XXX-*.md` files; `@UseCase`-annotated tests link back to specifications via an IntelliJ navigator plugin. Implementation is driven through Claude Code marketplace plugins — a stack-agnostic `aiup-core` plugin for requirements, modelling, and reverse-engineering, paired with a technology-specific `aiup-vaadin-jooq` plugin that targets Java web applications with Vaadin and jOOQ \[Martinelli2026\].

Key differences from Post-Coding Development: AIUP's *Determinism Fallacy* section explicitly argues that exhaustive specifications are neither possible nor necessary, and relies on iterative refinement plus a test suite to catch behavioural drift between regenerations; PCD takes the opposite stance — BEHAVIORs and INVARIANTS are intended to be precise, and EXAMPLES are the executable oracle that closes the loop. AIUP's technology-specific implementation plugins make its specifications stack-bound in practice (a Use Case for a Vaadin form is a Vaadin Use Case); PCD's deployment-template-and-hints layering exists specifically to keep specifications language- and platform-agnostic. AIUP addresses governance and traceability in generic enterprise terms but does not specify SPDX SBOM generation, signed packaging, build-system provenance, or pathways to ISO 26262, DO-178C, IEC 62304, or Common Criteria certification. Its tooling is distributed as Claude Code marketplace plugins installed at runtime; PCD's `mcp-server-pcd` embeds all assets at build time, eliminating the runtime download chain that supply-chain-sensitive deployments must avoid. The two paradigms target different audiences and are complementary: AIUP optimises for the productivity of Java-shaped enterprise application teams; PCD optimises for the audit trail of regulated and long-lived components where the specification must remain the sole certified artefact.

**Coding Agents as Normal Technology, Narayanan and Kapoor (Analysis, 2026)**
This entry is an analysis rather than a methodology, and so does not appear in the comparison table above. On 11 June 2026, Arvind Narayanan and Sayash Kapoor - authors of *AI as Normal Technology* - argued that AI has not displaced software engineers and will not, even in the profession where AI capability and adoption are furthest along. Their model is a *decide-execute-deliver sandwich*, with deep understanding of the codebase, the business, and the environment as a prerequisite for all three layers: AI has compressed the middle, execute layer almost completely, but the two ends resist automation. They marshal the empirical case that writing code was never the bottleneck - studies put coding at 9 to 61 percent of developer time, and the Demirer et al. data show large coding-activity gains attenuating to a 30 percent release increase - and they ground the structural claim in Fred Brooks' forty-year-old distinction between essential and accidental complexity: tooling reduces only the accidental kind, while specifying correct behaviour is the essential kind \[Narayanan2026, Demirer2026, Brooks1986\].

The relevance to Post-Coding Development is direct, because the three layers map onto the paradigm's placement of the human review point. The decide layer is the specification - the layer PCD makes the sole durable, human-authored artefact, and the layer it likewise holds to be the hard, non-automatable one. The execute layer is the generated code, which PCD demotes to a regenerable derivative. The deliver layer is where the tension sits, and it is the strongest external challenge to PCD's central move. Narayanan and Kapoor argue that the deliver layer resists compression because a team must comprehend and be accountable for what it ships, and that this is held in place not only by present model unreliability but by liability law and sector regulation that can be strengthened by choice. Read against PCD, that is an argument that human comprehension must stay at the code level - which is exactly the placement PCD rejects.

PCD's answer is that comprehension is relocated, not removed. The human comprehends and signs off on the specification; the generated code is held accountable by an auditable pipeline - compile gate, executable examples, independent test generation, optional formal verification, software composition analysis, and a cryptographic specification-to-artefact chain. This is the same move the analysis itself makes for the decide layer, where it notes that the value of human judgement migrates upward as lower decisions become delegable to AI; PCD applies that logic one layer further, migrating human comprehension from the code to the specification plus the pipeline. The honest residue is a single open question: is the pipeline auditable enough that a certifier will accept specification-level comprehension in place of code-level comprehension? That is precisely PCD's regulated-domain bet, and the part not yet proven at certification scale. The convergence - a labour-economics analysis and an engineering paradigm independently reaching for the specification as the layer that matters - is independent confirmation of the problem framing, not validation of PCD specifically; the authors developed the sandwich model with no knowledge of this work \[Narayanan2026\].
...
work product. The name marks that shift, not a phase that follows coding. That reading is reinforced, from the opposite direction, by recent labour-economics analysis arguing that engineers are not replaced but that the centre of their work moves away from writing code (see the Narayanan and Kapoor entry above).
...
| AI Unified Process (AIUP) | Use Case spec (UC-XXX) | Yes | No | No | No |
...
### Idea 2 (Implemented, expanded v0.3.23): Independent Test Generation

#### Rationale

The EXAMPLES section of a PCD specification is written by the spec author.
It tests what the author thought of. An independent test suite — produced
by a different LLM that has read only the specification — will generate
tests for cases the author did not consider: boundary conditions, error
...
language's standard testing framework. The suite asserts on every EXAMPLE,
...
| Action | When | Rationale required |
|---|---|---|
...
The per-EXAMPLE confidence table includes columns for `Verification method`
and `Unverified claims`. A claim is `High` confidence only if a named test
function in `independent_tests/<llm-name>/` passes without a live external
service; in dual-LLM mode, both suites must pass. A claim with no test
backing must be explicitly disclosed as unverified.
...
A claim is `verified` only if it references a specific test function that
passes without a live external service. Unverified claims must be listed
explicitly, not silently omitted.

#### Formal Rules for Independent Tests
...
1. **One test function per EXAMPLE.** Each EXAMPLE in the spec must have a
   corresponding `Test<CamelCasedExampleName>` function in the test file.
...
3. **Tests must pass without infrastructure.** `go test ./independent_tests/<llm-name>/`
   (or equivalent for the target language) must succeed with no live
   cluster, no live daemon, and no network access.
...
5. **Tests and implementation share the language.** Both are written in the
   language the deployment template resolves to, using the language's
   standard testing framework. Cross-language test harnesses are explicitly
   not used; the constraint is operational (certified runtime images carry
   one toolchain).

6. **Test directories are named for the LLM.** `independent_tests/<llm-name>/`,
...
as a `required` deliverable in every deployment template that has an
EXECUTION compile gate. Single-LLM mode produces one `<llm-name>/`
subdirectory; dual-LLM mode produces two. For `verified-library` and
templates where `Safety-Level` is above QM, dual-LLM mode is recommended;
for QM components, single-LLM mode is sufficient.
...
- \[Narayanan2026\] Arvind Narayanan and Sayash Kapoor, *"Why AI hasn't replaced software engineers, and won't"*, AI as Normal Technology, 11 June 2026. https://www.normaltech.ai/p/why-ai-hasnt-replaced-software-engineers

- \[Demirer2026\] Mert Demirer, Leon Musolff and Liyuan Yang, *"Writing Code vs. Shipping Code: Productivity Effects Across Generations of AI Coding Tools"*, NBER Working Paper No. 35275, May 2026. https://www.nber.org/papers/w35275

- \[Brooks1986\] Frederick P. Brooks Jr., *"No Silver Bullet - Essence and Accidents of Software Engineering"*, in Information Processing 86 (Proceedings of the IFIP Tenth World Computing Conference), Elsevier, 1986; reprinted in *The Mythical Man-Month*, anniversary edition, Addison-Wesley, 1995.
