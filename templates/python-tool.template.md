...
|-----|-------|------------|-------|
| VERSION | MAJOR.MINOR.PATCH or YYYY.MM.DD.VV | required | Semantic versioning (MAJOR.MINOR.PATCH) or dated versioning (YYYY.MM.DD.VV). |
...
| build | required | `Makefile` | Targets: `lint`, `typecheck`, `test`, `build`, `man`, `clean`. `man` target: `pandoc {TOOL_NAME}.1.md -s -t man -o {TOOL_NAME}.1`. |
| docs | required | `README.md` | Installation via OBS (zypper/apt/dnf). Must not document curl-based installation. |
| man | required | `{TOOL_NAME}.1.md`, `{TOOL_NAME}.1` | Markdown source converted to troff via `pandoc`. Section 1 (user commands). Install to `%{_mandir}/man1/` (RPM) and `usr/share/man/man1/` (DEB). |
...
- `%build` must include: `pandoc {TOOL_NAME}.1.md -s -t man -o {TOOL_NAME}.1`
- `%install` must use `pip install --no-deps --root %{buildroot}`
- `%files` must use `%{python3_sitelib}`, not `%{python3_sitearch}`
- `%files` must include: `%{_mandir}/man1/{TOOL_NAME}.1*`

**debian/control:**
- `Build-Depends` must include `pandoc`

**debian/rules:**
- Must include man page build step: `pandoc {TOOL_NAME}.1.md -s -t man -o {TOOL_NAME}.1`
- Man page must be installed to `usr/share/man/man1/{TOOL_NAME}.1`
